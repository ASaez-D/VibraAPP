...
class _CalendarScreenState extends State<CalendarScreen> {
                ...
                children: [
                  ...
                  Wrap(
                    ...
                    children: [
                      ...
                      _quickSelectButton("Mañana", DateTime.now().add(const Duration(days: 1))),
                      _quickSelectButton("Esta semana", null, rangeWeek: true),
                      _quickSelectButton("Próximos 30 días", null, rangeNext30Days: true),
                    ],
                  ),
  ...
  String _getButtonLabel() {
    final sd = startDate;
    final ed = endDate;
    if (sd == null && ed == null) return "ESCOGER FECHA";
    if (sd == null && ed != null) return DateFormat('d MMMM', 'es_ES').format(ed).toUpperCase();
    if (sd != null && ed == null) return DateFormat('d MMMM', 'es_ES').format(sd).toUpperCase();
    if (sd!.isAtSameMomentAs(ed!)) return DateFormat('d MMMM', 'es_ES').format(sd).toUpperCase();
    return "${DateFormat('d MMMM', 'es_ES').format(sd)} -> ${DateFormat('d MMMM', 'es_ES').format(ed)}".toUpperCase();
  }
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          gradient: isActive
              ...
              : null,
          color: isActive ? null : Colors.purpleAccent.withOpacity(0.2),
          borderRadius: BorderRadius.circular(30),
        ),
        child: Text(
          label,
      ...
      children: [
        ...
        GridView.builder(
          ...
          itemBuilder: (context, dayIndex) {
            ...
            final isPast = date.isBefore(DateTime(currentDate.year, currentDate.month, currentDate.day));
            final isSelectedStart = startDate != null && date == startDate;
            final isSelectedEnd = endDate != null && date == endDate;
            final isBetween = startDate != null && endDate != null && date.isAfter(startDate!) && date.isBefore(endDate!);
            final isToday = date == DateTime(currentDate.year, currentDate.month, currentDate.day);
            return GestureDetector(
              onTap: isPast ? null : () {
                HapticFeedback.lightImpact();
                setState(() {
                  if (isQuickSelection) {
                    ...
                    if (isSelectedStart) startDate = null;
                    else if (isSelectedEnd) endDate = null;
              ...
              child: Center(
                child: AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: isToday ? Border.all(color: Colors.purpleAccent, width: 1.5) : null,
                  ),
                  child: AnimatedScale(
                    ...
                    child: Text(
                      ...
                      style: TextStyle(
                        color: isPast
                            ? Colors.white38
                            : isSelectedStart || isSelectedEnd || isBetween
                                ? Colors.black
                                : Colors.white,
                        fontWeight: FontWeight.bold,
                      ),

...
class _ConcertDetailScreenState extends State<ConcertDetailScreen>
  ...
  /// Perfil del usuario cargado desde FirebaseAuth (funciona desde cualquier pantalla)
  Map<String, dynamic>? _localUserProfile;

  /// URL de entradas resuelta (puede venir del concierto o buscada en Ticketmaster)
  String _resolvedTicketUrl = '';
  ...
  void initState() {
    ...
    _resolvedTicketUrl = widget.concert.ticketUrl;
    ...
    _loadUserProfile();
    _loadMoreDates();
    if (widget.concert.ticketUrl.isEmpty) {
      _resolveTicketUrl();
    }
  }

  /// Lee el perfil del usuario desde FirebaseAuth para mostrar el avatar y drawer
  void _loadUserProfile() {
    ...
    if (firebaseUser != null) {
      ...
      // Fallback: usar el perfil pasado como parámetro (p.ej. Spotify)
      setState(() {
        _localUserProfile = widget.userProfile;
      });
    }
  }

  /// Si no hay ticketUrl guardada, intenta resolverla en Ticketmaster
  Future<void> _resolveTicketUrl() async {
    try {
      ...
      for (final r in results) {
        if (r.ticketUrl.isNotEmpty) {
          if (mounted) setState(() => _resolvedTicketUrl = r.ticketUrl);
          return;
        }
      }
    } catch (_) {}
  ...
  void _loadMoreDates() async {
    ...
    if (mounted) {
      setState(() {
        _relatedConcerts = results
            .where(
              (c) =>
                  c.date != widget.concert.date &&
                  c.venue != widget.concert.venue,
            )
            .toList();
  ...
  void _openMap(BuildContext context) {
    ...
    if (widget.concert.latitude != null && widget.concert.longitude != null) {
      googleMapsUrl = Uri.parse(
        "geo:${widget.concert.latitude},${widget.concert.longitude}?q=${widget.concert.latitude},${widget.concert.longitude}(${Uri.encodeComponent(widget.concert.venue)})",
      );
    } else {
      googleMapsUrl = Uri.parse(
        "https://www.google.com/maps/search/?api=1&query=${Uri.encodeComponent('${widget.concert.venue}, ${widget.concert.city}')}",
      );
    }
  ...
  void _toggleSave(AppLocalizations l10n) {
    ...
    _userDataService.toggleSaved(widget.concert.name, {
      ...
      'imageUrl': widget.concert.imageUrl,
      'venue': widget.concert.venue,
      'ticketUrl': widget.concert.ticketUrl,
      'priceRange': widget.concert.priceRange,
      'city': widget.concert.city,
      'address': widget.concert.address,
      'genre': widget.concert.genre,
    });
  ...
  Widget build(BuildContext context) {
    ...
    final String heroTagToUse =
        widget.heroTag ?? widget.concert.name + widget.concert.date.toString();
    ...
    return Scaffold(
          ...
          GestureDetector(
            ...
            child: Padding(
              ...
              child: CircleAvatar(
                ...
                backgroundImage: (_localUserProfile?['photoURL'] != null)
                    ? NetworkImage(_localUserProfile!['photoURL'])
                    : null,
                child: (_localUserProfile?['photoURL'] == null)
      ...
      endDrawer: _localUserProfile != null
          ? HomeDrawer(
              userProfile: _localUserProfile!,
                ...
                child: Column(
                  children: [
                    ...
                    _buildInfoRow(
                      ...
                      l10n.detailOrganizedBy(widget.concert.venue),
                      secondaryText,
                      primaryText,
                    ),
                  ],
                      ...
                      child: Column(
                        children: [
                          _buildInfoRow(
                            ...
                            "${l10n.detailDoorsOpen}: ${DateFormat('HH:mm').format(widget.concert.date)}",
                            secondaryText,
                            primaryText,
                          ),
                          const SizedBox(height: 16),
                              ...
                              Expanded(
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    Text(
                                      "${widget.concert.address}, ${widget.concert.city}",
                                      style: TextStyle(
                                        color: secondaryText,
                                        fontSize: 13,
                                      ),
                      ...
                      return GestureDetector(
                        onTap: () {
                          Navigator.push(
                            ...
                            MaterialPageRoute(
                              builder: (_) =>
                                  ConcertDetailScreen(concert: related),
                            ),
                          );
                        },
                        child: Container(
                            ...
                            children: [
                              ClipRRect(
                                ...
                                child: Image.network(
                                  ...
                                  fit: BoxFit.cover,
                                  errorBuilder: (c, e, s) =>
                                      Container(width: 80, color: iconBg),
                                ),
                              ),
      ...
      bottomNavigationBar: Container(
        ...
        decoration: BoxDecoration(
          color: scaffoldBg,
          border: Border(
            top: BorderSide(color: primaryText.withValues(alpha: 0.1)),
          ),
        ),
        child: SafeArea(
          top: false,
            ...
            children: [
              Column(
                ...
                children: [
                  ...
                  if (mainPrice != l10n.detailCheckPrices &&
                      mainPrice != l10n.detailFree)
                    Text(
                      l10n.detailCheckWeb,
              ...
              Expanded(
                child: SizedBox(
                  ...
                  child: ElevatedButton(
                    onPressed: _resolvedTicketUrl.isNotEmpty
                        ? () => _launchURL(_resolvedTicketUrl)
                        : null,
